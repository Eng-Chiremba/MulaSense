...
- **Chessmore Chiremba** - Team Leader, Full-Stack Developer, Techinical
- **Tinashe Chikoko** - Backend Developer, presenter
- **Anesu Manderbvu** - Research, testing, presendter
---
...
- Receive AI-powered financial coaching(Chatbot)
...
- **EcoCash Integration:** Ecocash API implementation, USSD codes (`*151#`, `*153#`), payment flows
...
### Tech Stack
We used an Incremental Software dev process to deliver 85% functional app
- **Backend:** Django 5.2.8 + Django REST Framework 3.15.2
- **Frontend:** React 18.3.1 + TypeScript 5.8.3
- **Mobile:** Capacitor 6.0 (Android/iOS)
- **AI:** OpenRouter API (Llama 3.1 + GPT-4 tier models), Ecocach
- **Payments:** EcoCash sandbox API,Mock APIs 
...
- AI (LLAMA) financial advisor trained on user summaries
...
###  Sandbox EcoCash API Integration
...
###  Micro-Business Intelligence for Young Entrepreneurs
...
MulaSense implements **Sandbox APIs** using the official REST endpoints with comprehensive error handling, automatic transaction recording, and seamless user experience.

**Base URL:** `https://mulasense.onrender.com/api/ecocash_pay/`
...
**Note:** Send Money and Buy Airtime use USSD execution, not API endpoints. innovation was to have an automatic trigger of the *151*1*1*{reciever}*{amount}# and user is only prompted to use enter pin..no dialing raquired
...
**Problem:** Ecocash does not have P@P APIs and Traditional USSD flows (*151#) require manual dialing, multiple menu navigation steps, and provide no transaction tracking or integration capabilities.

**Solution:** Integrated official EcoCash sandbox with hybrid USSD fallback for services like buy airtime and send money which are not supported with in the app .. the the app now automatically run the USSD (*151*) with all the prompt .. no need for user action.. just pin input 
